...
import os, numpy as np, cv2 as cv, keyboard, cv2, math
...
#for update temp on image 

def measureColorTemp():

   img = np.frombuffer(cameraData, dtype= np.uint8)
...
lower_color2 = np.array([3,20,103])
            ...
            if area > 200:
                ...
                if factor > 0.2:
                   ...
                   if 120 <= centerX <= 135:
                       ...
                       rightMidMotor.setVelocity(right)
                       rightRearMotor.setVelocity(right)
                    #img_contour = cv.drawContours(img, [cnt], -1, (0,255,0), 3)
